...
const MIN_SIZE    = 0.5  // minimum plot dimension in metres
const SNAP        = 0.5
  ...
  return (
    <g>
      <rect
        ...
        rx={4}
        fill={selected ? '#C8E89A' : '#DFF0C0'}
        stroke={selected ? '#3B6D11' : '#97C459'}
  ...
  return (
    <div className="flex flex-col gap-0 h-full">
      ...
      <p className="text-xs text-center py-2" style={{ color: '#5A7040' }}>
        Appui long sur le sol pour ajouter une parcelle. Glisse pour déplacer.
      </p>
      ...
      <div className="overflow-auto flex-1 flex items-start justify-center p-4">
        ...
        >
          ...
          {Array.from({ length: Math.floor(garden.width / SNAP) + 1 }, (_, i) => (
            <line
              key={`v${i}`}
              x1={i * SNAP * SCALE} y1={0}
              x2={i * SNAP * SCALE} y2={gh}
              stroke="#C8E89A" strokeWidth={0.5}
            />
          ))}
          {Array.from({ length: Math.floor(garden.height / SNAP) + 1 }, (_, i) => (
            <line
              key={`h${i}`}
              x1={0}  y1={i * SNAP * SCALE}
              x2={gw} y2={i * SNAP * SCALE}
              stroke="#C8E89A" strokeWidth={0.5}
            />
          ))}
          {/* Plots */}
      ...
      {selectedPlot && (
        <div
          ...
          style={{ background: '#F8FBF3', borderTop: '1px solid #DDE8CC' }}
        >
