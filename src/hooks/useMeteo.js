...
// Traduit un WMO weather code en emoji + label
...
export function useMeteo(regionId) {
  ...
  // Alertes dérivées du forecast
  const alertes = []
  if (meteo?.daily) {
    const { temperature_2m_min, precipitation_sum } = meteo.daily

    // Gel : température min < 2°C dans les 3 prochains jours
    ...
    // Sécheresse : 4 jours consécutifs sans pluie significative (< 1 mm)
    let secCount = 0
  ...
  // Pour chaque jour, y a-t-il de la pluie prévue (≥ 3 mm) ?
